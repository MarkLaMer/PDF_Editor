...
def save_signature() -> Any:
    ...
    try:
        # Load the image into Pillow and preserve transparency.
        # Save as PNG with alpha channel to maintain transparent backgrounds.
        with BytesIO(image_data) as bio:
            with Image.open(bio) as sig_img:
                # Always convert to RGBA to preserve transparency
...
def list_signatures() -> Any:
    ...
    files = []
    try:
        files = [f.name for f in SIGNATURE_FOLDER.iterdir() if f.is_file() and f.suffix.lower() == '.png']
    except Exception:
        pass
...
def list_fonts() -> Any:
    ...
    try:
        ...
        pass
    # Add default fonts
    fonts.append({'filename': 'default', 'name': 'Great Vibes (Default)'})
    fonts.append({'filename': 'helvetica', 'name': 'Helvetica'})
    return jsonify({'fonts': fonts})


@app.route('/fonts/<path:filename>')
def serve_font(filename: str) -> Any:
    """Serve a font file from the fonts folder."""
    font_path = FONTS_FOLDER / filename
    if font_path.exists() and font_path.is_file():
        return send_file(font_path, mimetype='application/font-sfnt')
    return jsonify({'error': 'Font not found'}), 404


@app.route('/delete_signature/<path:filename>', methods=['DELETE'])
def delete_signature(filename: str) -> Any:
    """Delete a signature file."""
    try:
        sig_path = SIGNATURE_FOLDER / filename
                    ...
                    if sig_type == 'saved':
                        ...
                        if sig_path and sig_path.exists():
                            ...
                            if sig_img is not None:
                                # Preserve transparency by keeping RGBA mode
                                if sig_img.mode != 'RGBA':
                                    sig_img = sig_img.convert('RGBA')
                                if ann_w and ann_h:
                                    draw_w = float(ann_w)
                                ...
                                # Use mask='auto' to preserve transparency
